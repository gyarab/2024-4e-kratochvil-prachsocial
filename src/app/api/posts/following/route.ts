...
/**
 * API Route pro ziskani prispevku od sledovanych uzivatelu
 * Pouziva se pro zobrazeni feedu "Following"
 */
export async function GET(req: NextRequest) {
  try {
    ...
    // Nacteni prispevku od sledovanych uzivatelu
    const posts = await prisma.post.findMany({
      where: {
        user: {
          followers: {
            some: {
              followerId: user.id, // Uzivatel je mezi followery autora prispevku
            },
          },
        },
      },
